...
## FASE 1: Conteinerização Imutável (`backend/`)

O foco inicial foi transformar o código-fonte em um artefato imutável, seguro e portátil.

### [Dockerfile](file:///c:/Projetos/.NET/ControlService/backend/Dockerfile)
...
### [build-and-push.sh](file:///c:/Projetos/.NET/ControlService/backend/build-and-push.sh)
...
## FASE 2: Infraestrutura de Aplicação (`backend/k8s/` e `infra-shared/k8s/`)

Migramos de contêineres isolados para uma malha orquestrada com alta disponibilidade.

### [01-redis-deployment.yaml](file:///c:/Projetos/.NET/ControlService/infra-shared/k8s/01-redis-deployment.yaml)
...
### [02-redis-service.yaml](file:///c:/Projetos/.NET/ControlService/infra-shared/k8s/02-redis-service.yaml)
...
### [03-app-deployment.yaml](file:///c:/Projetos/.NET/ControlService/backend/k8s/03-app-deployment.yaml)
...
### [04-app-service.yaml](file:///c:/Projetos/.NET/ControlService/backend/k8s/04-app-service.yaml)
...
## FASE 3: Observabilidade e Monitoramento (`infra-shared/k8s/`)

Implementamos o monitoramento baseando-nos nos **Golden Signals** (RED).

### [05-prometheus-pvc.yaml](file:///c:/Projetos/.NET/ControlService/infra-shared/k8s/05-prometheus-pvc.yaml)
- **Persistência**: Define um volume dedicado para que, mesmo que o Prometheus reinicie, o histórico de métricas não seja perdido.

### [06-prometheus-configmap.yaml](file:///c:/Projetos/.NET/ControlService/infra-shared/k8s/06-prometheus-configmap.yaml)
...
### [07-prometheus-deployment.yaml](file:///c:/Projetos/.NET/ControlService/infra-shared/k8s/07-prometheus-deployment.yaml)
- **Engine Analítico**: Gerencia o binário do Prometheus com montagem de volumes para o banco de dados de séries temporais.

### [08-prometheus-service.yaml](file:///c:/Projetos/.NET/ControlService/infra-shared/k8s/08-prometheus-service.yaml)
- **ClusterIP 9090**: Permite que o Grafana acesse os dados do Prometheus internamente.

### [09-grafana-deployment.yaml](file:///c:/Projetos/.NET/ControlService/infra-shared/k8s/09-grafana-deployment.yaml)
- **Camada de Visualização**: Injeta credenciais administrativas via env vars e configura o acesso automático ao datasource.

### [10-grafana-service.yaml](file:///c:/Projetos/.NET/ControlService/infra-shared/k8s/10-grafana-service.yaml)
- **Dashboard Port (30030)**: Porta externa para acesso ao painel visual de métricas.

### [grafana-dashboard.json](file:///c:/Projetos/.NET/ControlService/infra-shared/k8s/grafana-dashboard.json)
...
### [deploy.sh](file:///c:/Projetos/.NET/ControlService/deploy.sh)
...
## FASE 5: Engenharia de Performance (`backend/stress/`)

Validamos os limites do sistema antes de qualquer lançamento.

### [k6-stress-test.js](file:///c:/Projetos/.NET/ControlService/backend/stress/k6-stress-test.js)
...
### [run-stress-test.sh](file:///c:/Projetos/.NET/ControlService/backend/stress/run-stress-test.sh)
- **Modo Headless**: Removidos os inputs manuais para permitir que o k6 rode de forma autônoma dentro do Jenkins.

### [README.md (stress)](file:///c:/Projetos/.NET/ControlService/backend/stress/README.md)
...
1. **Desenvolvimento e Integração Contínua (Pipeline via Jenkins)**
   ...
   - A versão atualizada da aplicação `.NET` (previamente adaptada no `Program.cs` e `.csproj` para expor o provedor de telemetria `prometheus-net`) é submetida a um build otimizado Multi-Stage a partir das definições do `backend/Dockerfile`. O ambiente expõe nativamente a porta `/metrics`.
...
4. **Testes e Limites Arquiteturais Operacionais (Stress Testing)**
   - Pondo o sistema à prova definitiva e exigindo sua alta latência imposta em SLA, as automações estressantes (via simulações como o script `backend/stress/k6-stress-test.js` ou scripts pontuais dentro da pasta `stress`) intensificam carga concentrada interativa contra a respectiva rota de APIs liberada aos usuários exposta via NodePort.
