...
*   O operador visualiza unicamente o seu itinerário diário designado, as ordens de serviço correspondentes e os insumos que deve utilizar ou relatar na execução física.
...
```
 [Escritório/Web] ──(Agenda o Itinerário)──> [Backend C# API]
                                                 │
                                         (Sincronização Ativa)
                                                 ▼
 [Operador/Mobile] <──(Baixa Itinerário Local)─────── [Internet]
         │
...
### 1. Obtenção do Itinerário (Download)
*   **Ação:** O operador, antes de sair a campo ou sob uma rede estável (Wi-Fi/4G), aciona ativamente a sincronização no app.
*   **Resultado:** O aplicativo consome a API REST (.NET 10) e baixa todo o itinerário de tarefas agendado pelo Módulo Comercial para o seu dispositivo, salvando as informações estruturadas no banco de dados local **SQLite**.
*   **Atualizações:** Se houver alteração administrativa na retaguarda posterior, o operador deve acionar a sincronização novamente para obter o itinerário atualizado.
...
*   **Ação:** Ao concluir uma atividade ou o itinerário do dia, o operador ativa o upload de retorno sob uma conexão de internet estável.
