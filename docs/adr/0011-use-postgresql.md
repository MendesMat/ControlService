...
- **Uniqueness lives in the database** as the last line of defense:
  ...
  - "Normalized" means trimmed, lowercase and without accents, matching the rules in `docs/product/conventions.md`. The normalized column is filled by the application, so the rule is identical in C# and in the database.
- The many-to-many relationship between users and profiles is a join table, not an array column.
- Profile levels are stored as rows (`profile_id`, `screen_key`, `level`), with a missing row meaning `Denied` (see `docs/product/features/permission-profiles.md`).
