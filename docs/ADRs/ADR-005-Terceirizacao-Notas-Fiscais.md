...
## Contexto:
O faturamento da empresa exige a emissão de notas fiscais de serviço e/ou produto. A integração direta com as prefeituras brasileiras para emissão de NFS-e é caótica e instável. Além disso, as regras de negócio variam atreladas às alíquotas que os diferentes CNPJs (e municípios de atuação) possuem.

## Decisão:
Decidi que o ERP apenas enviará as informações do cliente faturado e os dados básicos (alíquotas já pré-configuradas no perfil do CNPJ) para um sistema SaaS terceiro especializado em notas (ex: eNotas, Focus NFe, PlugNotas). Esse terceiro assumirá a responsabilidade pela mensageria com a SEFAZ/Prefeitura e me disponibilizará o link do PDF/XML de retorno no final do fluxo.
