...
- Fluxos de trabalho: aprovação de propostas, agendamento de itinerários, faturamento, emissão de documentos.
...
- Fluxos de trabalho: receber o itinerário diário de serviços, executar as atividades (registrar insumos químicos utilizados, fotografar o ambiente, registrar não conformidades e dar baixa no serviço como concluído ou cancelado) e sincronizar os resultados com o ERP ao retornar a uma área com conectividade.
...
- O Módulo Operacional **conhece** a interface de agendamento do Módulo Comercial (para obter o itinerário).
...
- **Consistência eventual entre itinerário agendado e itinerário executado:** Existe uma janela de tempo em que o operador pode estar trabalhando com uma versão desatualizada do itinerário (caso mudanças tenham sido feitas no escritório após a sincronização inicial). Esse risco é aceito e mitigado por processo operacional (o operador deve sempre sincronizar no início do turno).
...
- O contrato da API de sincronização (endpoints de obtenção de itinerário e de envio de baixas) será documentado formalmente e versionado via URL (`/api/v1/sync/...`), garantindo que versões antigas do app continuem funcionando durante períodos de transição.
...
- Testes de integração cobrirão os cenários de borda da sincronização: itinerário não encontrado, baixa duplicada, e conflito de status entre campo e retaguarda.
