...
O processo de modelagem deste domínio não ocorreu de maneira isolada ou puramente acadêmica. A extração das regras de negócio e a consolidação do conhecimento foram conduzidas por mim, desenvolvedor e arquiteto do sistema, atuando simultaneamente como o **Especialista de Domínio Principal**.
...
O conhecimento do domínio é descentralizado e compartilhado entre diferentes perfis de especialistas dentro da nossa organização. A minha modelagem tática e estratégica foi baseada nas interações diárias e processos dos seguintes atores:

**1. Especialista em Regras de Negócio e Processos (Coordenador Administrativo)**
- **Papel:** Eu, como coordenador, tenho a visão macro do negócio, entendimento claro das falhas nos sistemas anteriores e sou responsável por orquestrar os fluxos entre setores da empresa.
- **Contribuição na Modelagem:** Defini as regras de transição de estado dos serviços no itinerário, modelei o multitenancy lógico (múltiplos CNPJs) e estruturei as regras de validação para a emissão de relatórios regulatórios (RAAE).

**2. Operadores Comerciais**
- **Papel:** Atuam no dia a dia do nosso escritório lidando diretamente com os clientes. São responsáveis pela captação de *leads*, agendamento de serviços, gestão do relacionamento, elaboração de propostas e todo o acompanhamento comercial.
...
- **Service Order (Ordem de Serviço):** Documento técnico gerado para um serviço, possuindo numeração sequencial independente e dividida por *Service Nature*. O polimorfismo desse documento é uma regra vital:
...
- **Execution Report (Baixa/Relato de Execução do Serviço):** Ação executada pelo Operador de Campo para atualizar o status do serviço. Trata-se do ato de marcar um serviço como concluído ou não concluído, em conjunto com o preenchimento de um relato sobre como foi a execução ou o motivo do cancelamento/impedimento.
...
3. **Contexto Operacional:** Execução no campo. Lida com as regras *offline-first*, uso de insumos químicos e efetivação das "baixas" de serviço.
4. **Contexto Financeiro:** Fluxo de caixa, recebíveis, pagáveis, cálculos de comissões e emissão de notas fiscais (serviço externo).
5. **Contexto de Relatórios:** Conformidade regulatória com o RAAE e auditorias de divergências (como propostas geradas com valor zerado, mesmo que de forma proposital).
...
- **Customer/Supplier (Upstream: Comercial → Downstream: Operacional, Financeiro e Relatórios):** O Contexto Comercial atua como o centralizador das Ordens de Serviço e Clientes. O Operacional (que envia as baixas), o Financeiro (que processa faturamentos manuais) e o módulo de Relatórios (que gera o RAAE) consomem as informações geradas pelo Comercial. Mudanças no Comercial exigem alinhamento e gestão cuidadosa com as necessidades de todos esses consumidores.
- **Conformist (Upstream: Gerenciamento → Downstream: Comercial):** O comercial deve ser conformar estritamente à injeção de *claims* de perfil e `tenant_id` fornecidos pelo Gerenciamento.
- **Anti-Corruption Layer (ACL):** Utilizado pelo Contexto Financeiro na comunicação com a API de Notas Fiscais externa. Enveloparei essa integração com interfaces do domínio (gateways), isolando o modelo financeiro interno das oscilações, regras e complexidades de cada prefeitura.
...
Sem a prospecção, o cadastro da proposta e o agendamento de serviços gerado no módulo Comercial, o módulo Operacional não tem itinerário para executar; o Financeiro não possui base para faturar; O módulo de Relatórios não possui insumos para formatar as exigências regulatórias (RAAE). O diferencial competitivo da organização depende intrinsecamente de um fluxo de vendas dinâmico, renovação proativa de garantias e um relacionamento blindado com o cliente.

Os demais contextos foram classificados taticamente como:
- **Supporting Domains:** Operacional (execução da atividade em campo) e Relatórios (Conformidade com INEA). Eles apoiam o serviço vendido.
- **Generic Domains:** Financeiro (pagáveis, recebíveis e nota fiscal) e Gerenciamento (autenticação e segurança), cujas lógicas de negócio poderiam ser facilmente delegadas a sistemas genéricos.
...
- **Raiz do Agregado (`Aggregate Root`):** `Order`. Ele atua como o orquestrador do trabalho comercial.
- **Entidades Internas:** `Service` (Serviço ou Etapa). Cada `Order` é composto por uma coleção de 1 a N `Services`, ou seja, uma única cobrança pode ser dividida em um serviço de N etapas. 
...
- Depende exclusivamente da porta `ICustomerRepository` via inversão de dependência, sem acoplamento com nenhum detalhe de infraestrutura.
...
3. **Transição de Estado (Execution Report):** O relato (*Execution Report*) de um serviço conclui o ciclo tático de campo. Ele engatilha a atualização de status do serviço no banco de dados e anexa todos os relatos técnicos da operação.
4. **Independência de Faturamento (Financeiro):** Uma decisão consciente de negócio é não atrelar o *Execution Report* ao faturamento automático. Como cada `Customer` possui particularidades contratuais severas, o módulo Financeiro atua de forma autônoma: o faturamento de uma `Order` fechada requer sempre a tomada de decisão humana de um operador financeiro.
5. **Garantia de Lisura (Relatórios):** Assim que o *Execution Report* ocorre, os dados fluem para o módulo de Relatórios de forma contínua, compondo gradualmente o RAAE. Para assegurar a lisura do processo real, o sistema permite a reversibilidade do relato (correção de informações inseridas erroneamente). O modelo reage a essa reversão re-computando o relatório ambiental de maneira consistente, sem ferir as leis do INEA.
...
- **Anti-Corruption Layer (ACL):** Integrações instáveis de terceiros, como a emissão de Nota Fiscal via API externa, serão isoladas usando portas e adaptadores (`INotaFiscalGateway`), protegendo o Bounded Context Financeiro das oscilações de APIs de prefeituras e garantindo uma linguagem uniforme dentro do domínio.
